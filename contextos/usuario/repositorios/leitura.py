...
class RepoLeituraUsuario(RepoLeituraBase[Usuario]):
    ...
    async def buscar_por_id(self, id: UUID) -> Usuario | None:
        return await self.sessao.scalar(select(Usuario).where(Usuario.id == id))

    async def buscar_por_email(self, email: str) -> Usuario:
        return await self.sessao.scalar(select(Usuario).where(Usuario.email == email))

    async def buscar_por_token(self, token: str) -> Usuario:
        return await self.sessao.scalar(select(Usuario).where(Usuario.token == token))
